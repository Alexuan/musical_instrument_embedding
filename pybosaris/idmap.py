...
class IdMap:
    ...
    def __eq__(self, other):
        ...
        assert(isinstance(other,IdMap))
        assert(other.validate())

        ok = self.leftids.shape == other.leftids.shape
        if not ok:
            return False

        ok = (self.leftids == other.leftids).all()
        ok &= (self.rightids == other.rightids).all()
    ...
    def read_txt(input_file_name):
        ...
        assert obj.validate(), "Wrong IdMap format"
        return obj
