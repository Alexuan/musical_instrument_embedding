...
from pybosaris.libmath import diff, ismember
from pybosaris.sidekit_wrappers import check_path_existance


__author__ = "Anthony Larcher and Andreas Nautsch" # andreas: just editing
...
class Scores:
    ...
    """
    def __init__(self, scores_file_name='', ndx=None):
        ...
        if scores_file_name == '':
            if ndx is not None:
                if isinstance(ndx, Ndx) or isinstance(ndx, Key):
                    self.modelset = deepcopy(ndx.modelset)
                    self.segset = deepcopy(ndx.segset)
                    self.scoremat = numpy.zeros((self.modelset.shape[0], self.segset.shape[0]))

                    if isinstance(ndx, Ndx):
                        self.scoremask = ndx.trialmask
                    else:
                        self.scoremask = (ndx.tar | ndx.non)
    ...
    def __repr__(self):
        ch = 'modelset:\n'
        ch += str(self.modelset)+'\n'
        ch += 'segset:\n'
        ch += str(self.segset)+'\n'
        ch += 'scoremask:\n'
        ch += str(self.scoremask) +'\n'
        ch += 'scoremat:\n'
        ch += str(self.scoremat) +'\n'
        return ch
    ...
    def align_with_ndx(self, ndx):
        ...
        if len(rindx) > 0 and len(cindx) > 0:
    ...
    def __eq__(self, other):
        ...
        ok = self.scoremat.shape == other.scoremat.shape
        if not ok:
            return False
        ...
        ok &= (self.scoremat[sort_idx0_model,:][:,sort_idx0_segs]
               [self.scoremask[sort_idx0_model,:][:,sort_idx0_segs]]
               == other.scoremat[sort_idx1_model,:][:,sort_idx1_segs]
               [other.scoremask[sort_idx1_model,:][:,sort_idx1_segs]]).all()
