...
class Key:
    ...
    def __eq__(self, other):
        ...
        ok = self.tar.shape == other.tar.shape
        if not ok:
            return False
        ...
        ok &= (self.tar[sort_idx0_model,:][:,sort_idx0_segs] == other.tar[sort_idx1_model,:][:,sort_idx1_segs]).all()
        ok &= (self.non[sort_idx0_model,:][:,sort_idx0_segs] == other.non[sort_idx1_model,:][:,sort_idx1_segs]).all()
    ...
    @staticmethod
    def read_matlab(input_file_fame):
        """Reads a Key object from an hdf5 file.
